...
public class Integrator1DExperiment {
	...
	public static void main(String[] args) {
		...
		final int numberOfEvaluationPoints = 10001;
		...
		final Integrator1D integratorSimpsons = new SimpsonsIntegrator1D(numberOfEvaluationPoints);
		testIntegrator(integratorSimpsons);

		final Integrator1D integratorSimpsonsWithStreams = new SimpsonsIntegrator1DWithStreams(numberOfEvaluationPoints);
		testIntegrator(integratorSimpsonsWithStreams);

		System.out.println();

		final Integrator1D integratorMonteCarlo = new MonteCarloIntegrator1D(numberOfEvaluationPoints, 3141);
		testIntegrator(integratorMonteCarlo);

		final Integrator1D integratorMonteCarloWithStreams = new MonteCarloIntegrator1DWithStreams(numberOfEvaluationPoints, 3141);
		testIntegrator(integratorMonteCarloWithStreams);

		System.out.println();

		final Integrator1D integratorRiemannLeftPoints = new QuasiMonteCarloIntegrator1D(numberOfEvaluationPoints);
		testIntegrator(integratorRiemannLeftPoints);

		final Integrator1D integratorRiemannLeftPointsWithStreams = new QuasiMonteCarloIntegrator1DWithStreams(numberOfEvaluationPoints);
		testIntegrator(integratorRiemannLeftPointsWithStreams);

		System.out.println();

		final Integrator1D integratorRiemannMidPoints = new RiemannMidPointIntegrator1D(numberOfEvaluationPoints);
		testIntegrator(integratorRiemannMidPoints);

		final Integrator1D integratorRiemannMidPointsWithStreams = new RiemannMidPointIntegrator1DWithStreams(numberOfEvaluationPoints);
		testIntegrator(integratorRiemannMidPointsWithStreams);

		System.out.println();

		final Integrator1D monteCarloIntegratorWithVanDerCorputSeq = new MonteCarloIntegrator1DFromRandomGenerator1D(numberOfEvaluationPoints, () -> new VanDerCorputSequence(2));
	...
	private static void testIntegrator(Integrator1D integrator) {
		...
		final double integralValueItegrator = integrator.integrate(integrand, lowerBound, upperBound);

		final double error = integralValueItegrator - integralValueAnalytic;
