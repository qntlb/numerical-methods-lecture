...
public class BermudanOptionExerciseInMonteCarloBlackScholesPlot {
	...
	public static void main(String[] args) throws Exception {
		...
		final double maturity1 = 1.0;	// T1
		final double strike1 = 120.0;	// K1

		final double maturity2 = 2.0;	// T2
		final double strike2 = 140.0;	// K2
		...
		final RandomVariable valueOption2InT2 = stockInT2.sub(strike2).floor(0.0);	// V2(T2) = max(S(T2)-K,0)
		final RandomVariable valueOption1InT1 = stockInT1.sub(strike1).floor(0.0);	// V1(T1) = max(S(T1)-K,0)
		final RandomVariable valueOption2InT1 = AnalyticFormulas.blackScholesOptionValue(stockInT1, riskFreeRate, volatility, maturity2-maturity1, strike2);	// V2(T1)
		...
		final var bermudanValue				= bermudanPathwiseValueAdmissible.mult(model.getNumeraire(initialTime)).expectation().doubleValue();
		final var bermudanValueWithForsight	= bermudanPathwiseValueForesight.mult(model.getNumeraire(initialTime)).expectation().doubleValue();
		 ...
		 * Plot the stuff
		 */
		final Plotable2D continuationValues			= PlotablePoints2D.of("Continuation Value max(S(T\u2082)-K\u2082, 0)",
				stockInT1, valueRelativeOption2InT2, new GraphStyle(new Rectangle(-1,-1,2,2), null, Color.RED));

		final Plotable2D expectedContinuationValue	= PlotablePoints2D.of("Black Scholes Value of Option on S(T\u2082)-K\u2082, conditional to T\u2081",
				stockInT1, valueRelativeOption2InT1, new GraphStyle(new Rectangle(-3,-3,5,5),null, Color.BLUE));

		final Plot plotCondExpEstimate = new Plot2D(
				List.of(
						continuationValues,
		...
		final Plotable2D exerciseValues = PlotablePoints2D.of("Underlying 1 (S(T\u2081)-K\u2081)",
				stockInT1, valueRelativeOption1InT1, new GraphStyle(new Rectangle(-3,-3,5,5), null, Color.GREEN));

		final Plotable2D bermudanPathwiseValueAdmissiblePlot = PlotablePoints2D.of("Bermudan pathwise value",
		...
		final Plotable2D bermudanPathwiseValueForesightPlot = PlotablePoints2D.of("Bermudan pathwise value with foresight",
