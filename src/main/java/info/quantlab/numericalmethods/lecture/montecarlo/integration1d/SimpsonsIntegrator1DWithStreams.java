...
public class SimpsonsIntegrator1DWithStreams implements Integrator1D {
	...
	public SimpsonsIntegrator1DWithStreams(int numberOfEvaluationPoints) {
		...
		if(numberOfEvaluationPoints % 2 != 1) {
			throw new IllegalArgumentException("numberOfEvaluationPoints should be odd");
		}
	}
	...
	public double integrate(DoubleUnaryOperator integrand, double lowerBound, double upperBound) {
		...
		final int numberOfDoubleSizeIntervals = (numberOfEvaluationPoints-1)/2;

		final double intervalSize  = domainSize/numberOfDoubleSizeIntervals / 2.0;			// the h in the formula

		// First, second and last evaluation point will be missing, added at the end
		final IntStream intervals = IntStream.range(1,numberOfDoubleSizeIntervals);

		final DoubleStream integralParts = intervals.mapToDouble(
				i -> 2 * integrand.applyAsDouble(lowerBound + 2*i*intervalSize)
				+ 4 * integrand.applyAsDouble(lowerBound + (2*i+1)*intervalSize));

		final double sum = integralParts.sum()
