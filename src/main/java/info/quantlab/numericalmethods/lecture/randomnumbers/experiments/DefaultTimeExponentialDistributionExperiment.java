...
public class DefaultTimeExponentialDistributionExperiment {
	...
	private static void plot(RandomNumberGenerator1D uniformSequence, double lambda, int numberOfSamples) {
		...
		final String title = "Exponential Distribution with " + uniformSequence.toString() + ", \u03bb = " + lambda;

		System.out.println(title);
		System.out.println("_".repeat(79));

		final DefaultTimeExponentialDistribution defaultTime = new DefaultTimeExponentialDistribution(uniformSequence, lambda);

		final double maturity = 5.0;		// T (in P(𝜏 > T) - survival probability)

		final List<Double> times = new ArrayList<>();
		double sumOfTimes = 0;
		long suvivalCounter = 0;
		for(int i=0; i<numberOfSamples; i++) {
			final double time = defaultTime.getNext();
		...
		final double survivalProbability = (double)suvivalCounter / numberOfSamples;
