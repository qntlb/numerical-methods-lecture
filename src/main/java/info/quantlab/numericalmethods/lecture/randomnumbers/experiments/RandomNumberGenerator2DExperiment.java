...
public class RandomNumberGenerator2DExperiment {
	...
	public static void main(String[] args) throws Exception {
		final RandomNumberGenerator generatorPseudo = new RandomNumberGeneratorFrom1D(new MersenneTwister(3141), 2);
		plotSamplePoints(generatorPseudo);

		final RandomNumberGenerator generatorQuasi1 = new RandomNumberGeneratorFrom1D(new VanDerCorputSequence(2), 2);
		plotSamplePoints(generatorQuasi1);

		final RandomNumberGenerator generatorQuasi2 = new HaltonSequence(new int[] { 2, 3 });
	...
	private static void plotSamplePoints(RandomNumberGenerator generator) throws Exception {
		...
		final String title = generator.toString();

		final Plot plot = Plots.createScatter(points, 0.0, 1.0, 0.0, 1.0, 3)
				.setTitle(title);
