...
public class MonteCarloIntegrationImplementationChecker {
	...
	public static boolean checkMonteCarloIntegrator(MonteCarloIntegrationAssignment solution) {
		...
		final MonteCarloIntegratorFactory integratorFactory = solution.getMonteCarloIntegratorFactory();

		for(final IntegratorTestCase testCase : List.of(new IntegratorTestCaseUnitCircle(), new IntegratorTestCaseNormalCDF())) {
			System.out.println("Testing " + integratorFactory.getClass().getCanonicalName() + " with " + testCase);

			final boolean successOfTestCase = checkMonteCarloIntegrator(integratorFactory, testCase);
	...
	public static boolean checkSimpsonsIntegrator(MonteCarloIntegrationAssignment solution) {
		...
		final IntegratorFactory integratorFactory = solution.getSimpsonsIntegratorFactory();

		for(final IntegratorTestCase testCase : List.of(new IntegratorTestCaseUnitCircle(), new IntegratorTestCaseNormalCDF())) {
			System.out.println("Testing " + integratorFactory.getClass().getCanonicalName() + " with " + testCase);

			final boolean successOfTestCase = checkSimpsonsIntegrator(integratorFactory, testCase);
	...
	public static boolean checkMonteCarloIntegrator(MonteCarloIntegratorFactory integratorFactory, IntegratorTestCase integratorTestCase) {
		...
		for(final long seed : new long[] { 3141, 6563, 132 }) {
			final long numberOfSamplePoints = 1000000;	// 1E-6

			final double integral = integratorFactory.getIntegrator(
	...
	public static boolean checkIntegralOfDoubleBinaryOperator(MonteCarloIntegrationAssignment solution) {
		...
		final double integral = solution.getIntegral((x,y) -> Math.sin(x)*Math.sin(y), 0, Math.PI, 0, Math.PI);
		final double integralAnalytic = 4.0;

		final boolean success = Math.abs(integral-integralAnalytic) < 5E-2;
	...
	public static boolean checkSimpsonsIntegrator(IntegratorFactory integratorFactory, IntegratorTestCase integratorTestCase) {
		...
		final long numberOfSamplePoints = (long) Math.pow(101.0, integratorTestCase.getDomain().getDimension());	// 101^d
		...
		final boolean success = Math.abs(integral-integratorTestCase.getIntegralAnalytic()) < 5E-3 * integratorTestCase.getToleranceScaling();
